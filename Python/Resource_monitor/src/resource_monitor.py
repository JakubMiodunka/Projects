...
class BarGraph:
    ...
    def __call__(self, value: int) -> None:
        ...
        # Errasing row where pointer is displayed
        self.stdscr.addstr(self.pointer_y, self.pointer_x, f"{'':25}")

        # Prinitng new pointer according to given value
...
def basic_init(stdscr: curses.window) -> None:
    ...
    # Returing graphs for further operations
    return cpu_graph, ram_graph

def basic_update(cpu_graph: BarGraph, ram_graph: BarGraph) -> None:
    # Updating cpu graph
    cpu_usadge = psutil.cpu_percent(interval=1)
    cpu_graph(cpu_usadge)
...
# Functions realted to advanced mode
def avanced_init(stdscr: curses.window) -> None:
    # Determining how many rows will be neaded to display all graphs
...
def avanced_update(cpu_graphs: tuple) -> None:
    # Collecting data about usadge of each CPU core
    usadge_per_cpu = psutil.cpu_percent(interval=1, percpu=True)

    # VAlidation if collected data corresponds to graf tuple given in argument
    assert len(cpu_graphs) == len(usadge_per_cpu), "Quantity of graphs do not match quantity of CPUs"

    # Updating each graph with corresponding value
    for graph, cpu_usadge in zip(cpu_graphs, usadge_per_cpu):
        graph(cpu_usadge)
...
def main(stdscr, advanced_mode: bool) -> None:
    ...
    try:
        if advanced_mode:
            ...
            cpu_graphs = avanced_init(stdscr)

            while True:
                basic_update(cpu_graph, ram_graph)
                avanced_update(cpu_graphs)
        else:
            while True:
                basic_update(cpu_graph, ram_graph)
