...
$package_installer "./Package_lists/general_use_packages.txt" || { $logger 2 "Failed to install specyfied packages - exit code ${$?}."; exit 3; }
...
mkdir -p /home/user/Shared || { $logger 2 "Failed to create mount point - exit code ${$?}."; exit 3; }
...
$startup_script_preparer "./Startup_scripts/vbox_mount.sh" || { $logger 2 "Failed to prepare specyfied script - exit code ${$?}."; exit 3; }
